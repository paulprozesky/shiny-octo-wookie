...
% constants
start_from = 113;
valid_gap = 512;  % apprently this must be a multiple of (xacclen * num_ant)
% correlator setup
num_ant = 4;
num_x = 32;
num_chans = 512;
xacc_len = 128;
% misc
sync_every_n_windows = 1;
n_consecutive_chans = 1;
sync_offset = 15;
% calculated
chans_per_x = num_chans / num_x;
block_len = num_ant * xacc_len * n_consecutive_chans;
blocks_per_window = chans_per_x / n_consecutive_chans;

if sync_offset == 0,
    sync_offset = chans_per_x;
end
assert(sync_offset <= chans_per_x, 'Sync cannot be offset past consecutive valid blocks.')
...
fprintf('Sync offset in window: %i\n', sync_offset);
fprintf('X-acc length: %i\n', xacc_len);
fprintf('Consecutive freqs per block: %i\n', n_consecutive_chans);
fprintf('Blocks per window: %i\n', blocks_per_window);
fprintf('Block length: %i\n', block_len);
fprintf('Gap between valid blocks: %i\n', valid_gap);
fprintf('Complete windows per sync pulse: %i\n', sync_every_n_windows);
fprintf('\n');
